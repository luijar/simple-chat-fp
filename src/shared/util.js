...
export function foldM(M) {
  ...
  return arr => arr.reduce((acc, m) => acc.concat(m), M.empty())
}

export function transducer(applicative, transformers) {
  // TODO: Check contract has applicative contract
  return val =>
    transformers.map(applyTo(val)).reduce((acc, a) => acc.ap(a), applicative)
}
...
export const on = curry((event, handler, handle) =>
  handle.on(event, handler(handle))
)

export const composeMessage = curry((name, msg) =>
